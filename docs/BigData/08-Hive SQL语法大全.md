...
SELECT expr, ... FROM tb_name ORDER BY col_name [ASC | DESC]
(A | B | C)
```
...
**示例：创建 hive_test2 数据库，并自定义HDFS的存储位置。**

```sql
create database if not exists hive_test2 location '/hive_test2';
```

新表的存储位置将位于 HDFS 文件系统的 `/hive_test2` 目录。
...
**示例：修改 hive_test2 数据库的存储位置：**

```sql
ALTER DATABASE hive_test2 SET LOCATION 'hdfs://master:8020/user/hive/warehouse/hive_test2/';
```
...
`ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t';`：指定分隔符，在 HDFS 上，表的数据存储内容，表示以 \t 分隔。

- **内部表**：未被 external 关键字修饰的即是内部表， 即普通表。 内部表又称管理表 , 内部表数据存储的位置由
  `hive.metastore.warehouse.dir` 参数决定（默认：`/user/hive/warehouse`），删除内部表会**直接删除**元数据及存储数据，因此内部表不适合和其他工具共享数据。
...
- **外部表**： `[EXTERNAL]`，是指表数据可以在任何位置，通过 `[LOCATION 'path']` 关键字指定。 数据存储的不同也代表了这个表在理念是并不是 Hive 内部管理的，而是可以随意临时链接到外部数据上的。所以，在删除外部表的时候，仅仅是删除元数据（表的信息），**不会删除数据本身**。
  - 可以先有表，然后把数据移动到表指定的 LOCATION 中
  - 可以先有表，然后把数据移动到表指定的 LOCATION 中

```sql
CREATE EXTERNAL TABLE test_ext(id int)
  LOCATION 'hdfs://master:8020/tmp/test_ext';
  ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t'
```

- **分区表**：基于列分区 `[PARTITIONED BY(col_name, col_type, ......)]` 


```sql
CREATE TABLE test_ext(id int)
  PARTITION BY(year string, month string, day string)
  ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t';
```

- **分桶表**：基于列分桶 `[CLUSTERED BY(col_name, col_type, ......)]`
...
```sql
CREATE TABLE tb_name_name LIKE other_tb_name;
```
...
```sql
CREATE TABLE tb_name_name AS SELECT ...;
```
...
```sql
ALTER TABLE tb_name SET tb_namePROPERTIES(key=value);
-- 常用属性
...
```sql
LOAD DATA [LOCAL] INPATH 'path' INTO TABLE tb_name PARTITION(partition_key='partition_value');
```
...
```sql
INSERT (OVERWRITE | INTO) TABLE tb_name PARTITION(partition_key='partition_value') SELECT ... FROM ...;
```
...
```sql
INSERT (OVERWRITE | INTO) TABLE tb_name 
	[PARTITION(partition_key='partition_value')] 
...
```sql
LOAD DATA [LOCAL] INPATH 'path' INTO TABLE tb_name [PARTITION(partition_key='partition_value')];	-- 指定分区可选
```
